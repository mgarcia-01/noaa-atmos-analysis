...
```
This table shows the event types ordered by frequency. This depicts that the most frequent types `r eventDamag[1,2]` and `r eventDamag[2,2]` are not the most fatal or the most injuring.
...
propDmg <- aggregate(repdata$PROPDMG, list(repdata$EVTYPE,repdata$PROPDMGEXP), sum)
colnames(propDmg) <- c("EVTYPE","EXP","PROPDMG")
cropDmg <- aggregate(repdata$CROPDMG, list(repdata$EVTYPE,repdata$CROPDMGEXP), sum)
colnames(cropDmg) <- c("EVTYPE","EXP","CROPDMG")
eventDamagProp <- merge(eventDamag, propDmg,by.x = "EVTYPE", all = TRUE)
eventDmgPrp <- merge(eventDamagProp, cropDmg,by = c("EVTYPE","EXP") , all = FALSE)
e
ventDmgPrp$TOTLDMG <- (eventDmgPrp$PROPDMG + eventDmgPrp$CROPDMG)

# this is the Millions
 a <- eventDmgPrp[which(eventDmgPrp$EXP == "M"),]
 a[order(a$TOTLDMG, decreasing = TRUE),]
#eventDmgPrp[which(eventDmgPrp$PROPDMG == max(eventDmgPrp$PROPDMG) ),]
