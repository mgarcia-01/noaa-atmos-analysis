...
tmEvent <- tmEvent[,c("FATALITIES","BGN_DATE")]
tmEvent$BGN_DATE <-  as.Date(tmEvent$BGN_DATE, format = "%m/%d/%Y")
tmEvent <- tmEvent[order(tmEvent$BGN_DATE, decreasing = FALSE),]

tmEventAg <- aggregate(tmEvent$FATALITIES, by=list(tmEvent$BGN_DATE), sum, na.rm = TRUE)      # repdata$BGN_DATE
